...
object KeeShareContainer {
    ...
    /**
     * Atomically write a DatabaseKDBX container to a file.
     *
     * Writes to a temporary file in the same directory, then renames to the
     * target path. This guarantees that the target file is never in a
     * partially-written state, which prevents Syncthing from syncing a
     * corrupt/incomplete container to other devices.
     *
     * @param database The database to write
     ...
     */
    @Throws(IOException::class)
    fun writeUnsignedAtomic(
        database: DatabaseKDBX,
        targetFile: File,
        password: String
    ) {
        val parentDir = targetFile.parentFile
            ?: throw IOException("Target file has no parent directory: $targetFile")

        if (!parentDir.exists()) {
            parentDir.mkdirs()
        }

        val tempFile = File.createTempFile(".${targetFile.nameWithoutExtension}_", ".tmp", parentDir)
        try {
            tempFile.outputStream().use { outputStream ->
                writeUnsigned(database, outputStream, password)
            }

            // Atomic rename — on POSIX filesystems this is atomic if src and dst
            // are on the same filesystem (which they are, same directory).
            if (!tempFile.renameTo(targetFile)) {
                // Fallback: copy + delete (less atomic, but handles cross-filesystem edge cases)
                tempFile.copyTo(targetFile, overwrite = true)
                tempFile.delete()
                Log.w(TAG, "renameTo failed, fell back to copy+delete for $targetFile")
            }
        } catch (e: Exception) {
